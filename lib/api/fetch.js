...
export function getTradeHistoryByAddress({ queryKey }) {
  const { etag, address, includeAssetInfo = true } = queryKey[1]
  const path = `/algodex-backend/trade_history.php?ownerAddr=${address}&getAssetInfo=${includeAssetInfo}`

  return fetchConditionalResource(path, etag)
}

export function getAssetsByAddress({ queryKey }) {
  const { etag, address } = queryKey[1]
  const path = `/algodex-backend/wallet_assets.php?ownerAddr=${address}`
