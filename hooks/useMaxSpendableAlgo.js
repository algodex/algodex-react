 ...
 */
async function getMinWalletBalance(algodex, accountInfo, includesFullAccountInfo = false) {
  if (!includesFullAccountInfo) {
    try {
      // get full account info
      accountInfo = await algodex.http.indexer.fetchAccountInfo(accountInfo)
    } catch (e) {
...
export const useMaxSpendableAlgoNew = (wallet) => {
  const {
    data,
