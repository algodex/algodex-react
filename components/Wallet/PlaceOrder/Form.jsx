...
export function PlaceOrderForm({ showTitle = true, asset, onSubmit, components: { Box }, selectedOrder }) {
  ...
  const isFormActive = typeof order !== 'undefined' && typeof wallet !== 'undefined' && isConnected && showForm
  return (
    <Box
      sx={{
        width: '100%',
        ...
        <Box>
          ...
          <Box sx={{ opacity: `${isFormActive ? '1' : '0.3'}` }}>
            <Form onSubmit={isFormActive && handleSubmit} className="overflow-x-scroll" disabled={isActive}>
              ...
              {isFormActive && <AvailableBalance wallet={wallet} asset={asset} />}
              <Tabs
                sx={{ marginBottom: '16px' }}
                textColor="primary"
                tabtype={order.type === 'buy' ? 'buy' : 'sell'}
                onChange={(e, value) => isFormActive && handleMarketTabSwitching(e, value)}
                value={tabSwitch}
              >
                <Tab label={t('limit')} />
                <Tab label={t('market')} />
              </Tabs>
              {isFormActive && !hasBalance && (
                <Typography color="gray.500" textAlign="center" className="m-8">
                  {t('insufficient-balance')}
                </Typography>
              )}
              <TradeInputs
                handleChange={isFormActive && handleChange}
                updateAmount={isFormActive && handleSlider}
                sliderPercent={sliderPercent}
                order={order}
                isFormActive={isFormActive && isFormActive}
                onChange={isFormActive && handleChange}
                asset={asset}
              ...
              <Button
                ...
                fullWidth
                disabled={
                  asset.isGeoBlocked || !hasBalance || order.total === 0 || isBelowMinOrderAmount || !isFormActive
                }
              >
