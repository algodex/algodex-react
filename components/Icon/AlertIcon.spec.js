...
describe('Icon Button', () => {
  it('Should render icon with the right props', () => {
    const { queryByTestId } = render(<AlertIcon data-testid="alert-icon" color="gray" flex={true} size={10} />)
  ...
  it('Should render icon diffently when flex is set to false', () => {
    const { queryByTestId } = render(<AlertIcon data-testid="alert-icon" color="gray" flex={false} size={10} />)
